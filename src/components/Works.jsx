    ...
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt
        options={{ 
          ...
          scale: 1,
          speed: 450 }}
        className="bg-tertiary p-5 rounded-2xl xs:w-[250px] sm:w-[360px] "
      >
        <div className="relative w-full h-[230px] xs:h-[230px]">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <p key={tag.name} className={`text-[14px] ${tag.color}`}>#{t(tag.name)}</p>
          ))}
        </div>
      </Tilt>
    </motion.div>
